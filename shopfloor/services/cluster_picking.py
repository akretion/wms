...
class ClusterPicking(Component):
    ...
    * Full bin: declaring a full bin allows to move directly to the first phase
    ...
    def _response_for_start_product(self, batch, product, message=None):
        return self._response(
            next_state="start_product",
            data=self._data_for_product_scan(batch, product),
            message=message,
        )

    def _response_for_scan_product_destination(self, batch, product, message=None):
        return self._response(
    ...
    def scan_product(self, picking_batch_id, barcode):
        ...
        * start_product: when the barcode does not match
        ...
        if not batch.exists():
            return self._response_batch_does_not_exist()
        product = self._find_next_product(batch)
        if not product:
        ...
        return self._response_for_scan_product_destination(batch, product)

    def scan_product_destination_pack(
        self, picking_batch_id, product_id, barcode, quantity
    ):
        ...
        * unload_single: when all lines are done and have different destinations
        ...
        lines = self._aggregatable_lines(batch).filtered(
    ...
    def _find_next_product(self, batch):
        """Find the first product with aggregatable lines, by picking sequence."""
        lines = self._aggregatable_lines(batch)
        if not lines:
            return self.env["product.product"]
        return fields.first(lines).product_id

    def _data_for_product_scan(self, batch, product):
        ...
        data = {
            ...
            "quantity": total_qty,
            "location_src": self.data.location(first_line.location_id),
            "lines": [
                {
                    "id": line.id,
    ...
    def _pick_next_state(self, batch, product=None, total_qty=0, bin_package=None):
        ...
        * unload_all / unload_single: when all lines are done
        ...
        if self.work.menu.pick_by_product:
            next_product = self._find_next_product(batch)
            if next_product:
                return self._response_for_start_product(
                    batch, next_product, message=message
                )
